...
def parse_on_conflict_columns(text: str) -> list[str]:
    m = re.search(r"ON\s+CONFLICT\s*\((.*?)\)", text, flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return []
    return [c.strip().strip('"').strip("'") for c in m.group(1).split(",") if c.strip()]

def extract_insert_columns(text: str) -> list[str]:
    m = re.search(r"INSERT\s+INTO\s+region_features\s*\((.*?)\)\s*VALUES", text, flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return []
    return [c.strip().strip('"').strip("'") for c in m.group(1).split(",") if c.strip()]
...
def main(argv=None) -> int:
    ...
    out_path = repo_root / "_ops_logs" / f"repository_contract_{args.table}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    text = repo_file.read_text(encoding="utf-8")
    on_conflict_cols = parse_on_conflict_columns(text)
    insert_cols = extract_insert_columns(text)
